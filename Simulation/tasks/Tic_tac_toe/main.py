...
def move_chess_pipeline(chess,chessboard,action):
    pos = chess.get_position()
    # get the pos to put
    put_pos = np.array(chessboard.get_position()) + chessboard_bias_position(action)
    # get
    franka.clear_path = True
    pos[2] += 0.1
    franka.move(env,pos,euler=[0,np.radians(180),0])
    pos[2] -= 0.1
    franka.move(env,pos,euler=[0,np.radians(180),0])
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if turn == 'player':
            ...
            move_chess_pipeline(chess,chessboard,action)
